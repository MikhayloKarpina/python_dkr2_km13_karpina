...
def get_word_score(word, n):
    ...
    count = []
    for i in word.lower():
        count.append(SCRABBLE_LETTER_VALUES[i])
    count = sum(count)
    rest = max(1, 7 * len(word) - 3 * (n - len(word)))
    return rest * count 
...
def update_hand(hand, word):
    ...
    for i in word.lower():
        if update_hand.get(i, 0) > 0:
            update_hand[i] = update_hand.get(i, 0) - 1
    return update_hand
...
def play_hand(hand, word_list):
    ...
    * The hand finishes when there are no more unused letters.
    ...
    while calculate_handlen(hand) > 0:
        ...
        display_hand(hand)
        # Ask user for input
        word = input("Enter a letter or ''!!'' to indicate that you finished: ")
